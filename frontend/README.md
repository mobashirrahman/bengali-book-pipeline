...
Request failures stay explicit: the typed adapter normalizes network errors into `ApiError` (status `0`) while preserving `AbortError` cancellation, and every view separates loading, empty, and failure states with retry actions where a retry can help. Search and work-detail requests are guarded so a stale response can never overwrite newer content. The API contract is unchanged — same `VITE_API_BASE_URL` base, same `/v2` endpoints, no invented routes; the home shelf passes the additive `has_documents` filter on `/v2/works` so it lists readable works first, then hydrates each work's accepted PDF/EPUB documents. Discover browses that same readable-works listing when the query is empty, de-duplicates `/v2/search` work/edition rows to one book per work, and applies its Readable-only toggle as the additive `has_documents` filter on `/v2/search`.
